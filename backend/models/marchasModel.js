...
export const crearMarcha = async (marcha) => {
  ...
  const [result] = await pool.query(
    `INSERT INTO marchas (titulo, anio, descripcion, duracion_segundos, tipo)
     VALUES (?, ?, ?, ?, ?)`,
...
export const actualizarMarcha = async (id, data) => {
  await pool.query(
    `UPDATE marchas 
     SET titulo=?, anio=?, descripcion=?, duracion_segundos=?, tipo=?, dedicatoria=?, updated_at=NOW() 
     WHERE id=?`,
    [
      data.titulo,
      data.anio,
...
export const eliminarMarcha = async (id) => {
